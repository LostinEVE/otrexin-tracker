class HomeController < ApplicationController
  def index
    ...
    maintenance_scope = current_user.maintenances
    maintenance_scope = maintenance_scope.where(truck: truck_filter) if truck_filter
    ...
    # Performance metrics from actual trip mileage records
    @total_miles = Mileage.total_miles(mileage_scope)
    @rpm = Mileage.overall_revenue_per_mile(mileage_scope)
    operating_cost_total = expense_scope.sum(:amount).to_f + maintenance_scope.sum(:cost).to_f
    @cpm = @total_miles.positive? ? (operating_cost_total / @total_miles).round(4) : nil
